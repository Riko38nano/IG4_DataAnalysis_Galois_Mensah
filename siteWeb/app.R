...
data = cbind(data, `A 2-roues motorisé - Dans les mois qui vont suivre`)
listeVar = append(listeVar, "A 2-roues motorisé - Dans les mois qui vont suivre", after = length(listeVar))
...
##-------------------------------------------------------------------------------------------------------------##
##--Récupérer les variables qui ont un lien avec la var qui associe le fait qu'on ira au travail avec son 2RM--##
##-------------------------------------------------------------------------------------------------------------##
##-------------------------------------------------------------------------------------------------------------##

testCHI2 <- function(v){
  x <- `A 2-roues motorisé - Dans les mois qui vont suivre`
  mytable = table(x, v)
...
liste_Var_Test_Chi2_Ok = c()

for(colonne in 1:dim(data)[2]){
  if(colnames(data)[colonne] != "A 2-roues motorisé - Dans les mois qui vont suivre"){
    if(testCHI2(data[,colonne]) == TRUE){
      liste_Var_Test_Chi2_Ok = append(liste_Var_Test_Chi2_Ok, colonne, after = length(liste_Var_Test_Chi2_Ok))
    }
  }
}

dataSansVar_les_mois_qui_vont_suivre = data[,-which(colnames(data)=="A 2-roues motorisé - Dans les mois qui vont suivre")]

dataTestOk = data[,liste_Var_Test_Chi2_Ok]
...
##----------------------------------------------------------------------------------------------------------------------------------##
##------------------------------------------------Analyse univarié tableau recapitulatif-----------------------------------------------------------##
##----------------------------------------------------------------------------------------------------------------------------------##
##----------------------------------------------------------------------------------------------------------------------------------##


tableauRecapEffectifFrequence <- function(v){
  nomCol= levels(v)
  nomLig=c("Effectif","Fréquence")
  
  matEffectifFrequence = matrix(data=NA,nrow = 2,ncol=length(nomCol))
  
  colnames(matEffectifFrequence)=nomCol
  rownames(matEffectifFrequence)=nomLig
  
  for(colonne in 1:ncol(matEffectifFrequence)){
    matEffectifFrequence[1,colonne] = round(length(which(v == nomCol[colonne])))
    matEffectifFrequence[2,colonne] = round((matEffectifFrequence[1,colonne] / length(v))*100)
  }
  matEffectifFrequence
}
...
##----------------------------------------------------------------------------------------------------------------------------------##
##------------------------------------------------Visualition bivarié masaic plot-----------------------------------------------------------##
##----------------------------------------------------------------------------------------------------------------------------------##
##----------------------------------------------------------------------------------------------------------------------------------##
...
listeSelectAnalyse = c("Impact financier", "Moyen de transport avant le confinement", "Activités pendant le confinement", "Besoin/ frustration")


arbreStat <- base64enc::dataURI(file="arbrestat.png", mime="image/png")
arbre <- base64enc::dataURI(file="arbre.png", mime="image/png")


sidebar <- dashboardSidebar(
  sidebarMenu(
    menuItem("Univarie", tabName = "1", icon = icon("th")),
    menuItem("Bivarie", icon = icon("th"), tabName = "2"),
    menuItem("Test Chi 2", tabName = "3", icon = icon("th")),
    menuItem("Analyse", tabName = "4", icon = icon("th")),
    menuItem("Arbre de decision", tabName = "5", icon = icon("th"))
  )
)
...
body <- dashboardBody(
  tabItems(
    tabItem(tabName = "1",
            fluidRow(
              box(selectInput("selectVarUnivarie", h3("Variables"), 
                              choices=colnames(data)), width=12),
              box(plotOutput("plotUnivar", height = 250),width=12),
              
              box(
                verbatimTextOutput("statsUnivarie"), width = 12)
            )
    ),
    
    tabItem(tabName = "2",
            fluidRow(
              box(selectInput("selectVarBivarie1", h3("Variable 1"), 
                              choices=colnames(data)), width=6),
              box(selectInput("selectVarBivarie2", h3("Variable 2"), 
                              choices=colnames(data)), width=6),
              box(plotOutput("plotBivarie"),width=12)
            )
    ),
    
    tabItem(tabName = "3",
            box(selectInput("selectVarPourTestChi2", h3("Variable"), 
                            choices=colnames(dataSansVar_les_mois_qui_vont_suivre)), width=12),
            box(
              verbatimTextOutput("resulTestChi2"), width = 12),
            hr(),
            box(verbatimTextOutput("conclusionTestChi2"),width = 12, background = "black"),
            DT::dataTableOutput("table")
    ),
    
    tabItem(tabName = "4",
            fluidRow(
              box(selectInput("selectAnalyse", h3("Analyse Intermediaire"), 
                              choices = listeSelectAnalyse), width=12),
              box(plotOutput("plotAnalyseInter"),width=12),
              hr(),
              box(verbatimTextOutput("conclusionTestInter"),width = 12, background = "black")
            )
    ),
    tabItem(tabName = "5",
            fluidRow(
              box(img(src=arbreStat),width=12),
              hr(),
              box(img(src=arbre,  width="1100", height="500" ),width=12)
            )
    )
  )
)
...
ui <- dashboardPage(
  dashboardHeader(title = "Projet Data Science"),
  sidebar,
  body
)
...
server <- function(input, output) {
  output$plotUnivar <- renderPlot({
    grapheUnivar(data[,which(colnames(data)==input$selectVarUnivarie)],"","")
  })
  output$statsUnivarie <- renderPrint({
    tableauRecapEffectifFrequence(data[,which(colnames(data)==input$selectVarUnivarie)])
  })
  output$plotBivarie <- renderPlot(
    {
      graphe_mosaic(data[,which(colnames(data)==input$selectVarBivarie1)], data[,which(colnames(data)==input$selectVarBivarie2)], "", "variable 1", "variable 2")
    }
  )
  output$resulTestChi2 <- renderPrint({
    x = data[,which(colnames(data)=="A 2-roues motorisé - Dans les mois qui vont suivre")]
    chisq.test(table(x, data[,which(colnames(data)==input$selectVarPourTestChi2)]))
  })
  output$conclusionTestChi2 <- renderPrint({
    x = data[,which(colnames(data)=="A 2-roues motorisé - Dans les mois qui vont suivre")]
    y = data[,which(colnames(data)==input$selectVarPourTestChi2)]
    if(chisq.test(table(x, y))$p.value < 0.05){
      "Il y a un lien entre les deux variables"
    }else{
      "Il n'y a pas de lien entre les deux variables"
    }
  })
  output$table <- DT::renderDataTable({
    `Liste des variables ayant un lien avec la variable qui associe aller au travail en 2RM apres le confinement` = colnames(dataTestOk)
    as.data.frame(`Liste des variables ayant un lien avec la variable qui associe aller au travail en 2RM apres le confinement`)
  })
  
  output$plotAnalyseInter <- renderPlot(
    {
      if(input$selectAnalyse == "Impact financier"){
        ...
        fviz_mca_var(ACMSegImpactFin, col.var = "contrib",
                     ...
                     invisible = "ind",
                     repel = TRUE)
      }else if(input$selectAnalyse == "Moyen de transport avant le confinement"){
        ...
        ACMSegToday = MCA(donneesTravailleur[vecUsage], graph = FALSE)
        fviz_mca_var(ACMSegToday, col.var = "contrib",
                     gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
                     invisible = "ind",
                     repel = TRUE)
      }else if(input$selectAnalyse == "Activités pendant le confinement"){
        ...
        fviz_mca_var(ACMSegActivites, col.var = "contrib",
                     ...
                     invisible = "ind",
                     repel = TRUE)
      }else if(input$selectAnalyse == "Besoin/ frustration"){
        data = rename.variable(data, 
  ...
  output$conclusionTestInter <- renderPrint({
    if(input$selectAnalyse == "Impact financier"){
      "Elle nous montre que ce sont les personnes dont les revenus ont été inchangés ou qui ont diminué, les plus susceptibles de se rendre au travail en 2-roues."
    }else if(input$selectAnalyse == "Moyen de transport avant le confinement"){
      "Ce graphique met en évidence le fait que les répondants en majorité gardent leurs habitudes de transports et aussi que le fait d’avoir adopté ou non son 2-roues comme moyen de transport pendant le confinement ne conditionne pas le choix de transport après le confinement. "
    }else if(input$selectAnalyse == "Activités pendant le confinement"){
      "Cette analyse est sur les activités bricolage et regarder des vidéos en lien avec la moto qui ont augmenté ou non pendant le confinement. On observe que le fait d’avoir fait plus de ces activités pendant le confinement n’a pas conditionné le fait d’aller en moto au travail après le confinement, au contraire ce sont les personnes qui n’ont pas augmenté leurs activités qui ont le plus été au travail en moto. Il nous montre aussi que les participants font soit toutes les activités soit aucune. "
    }else if(input$selectAnalyse == "Besoin/ frustration"){
      "Cette analyse nous montre que les personnes qui se sont senti frustré de ne pas rouler ont aussi senti le besoin de rouler et ainsi augmente les chances qu’ils se rendent au travail en moto après le confinement."
    }
